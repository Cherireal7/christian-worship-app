...
import { getPrayerById } from '../../constants/prayers';
    ...
    return (
      <SafeAreaView edges={['top', 'left', 'right', 'bottom']} style={[styles.safeArea, { backgroundColor: theme.background }]}>
        <View style={styles.missingWrap}>
          <Text style={[styles.missingTitle, { color: theme.text }]}>ጸሎቱ አልተገኘም</Text>
          <Pressable onPress={() => router.replace('/other-prayers')} style={styles.backHomeButton}>
            <Text style={styles.backHomeText}>ወደ ጸሎቶች ተመለስ</Text>
          </Pressable>
        </View>
      </SafeAreaView>
    );
      ...
      <View style={[styles.screen, { backgroundColor: theme.background }]}>
        <ScrollView contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false}>
          <View style={styles.headerRow}>
            ...
            <Text style={[styles.headerTitle, { color: theme.text, fontSize: 20 * appFontScale }]} numberOfLines={1}>
              {prayer.title}
            </Text>
            <Pressable
          ...
          <View style={[styles.readerCard, { backgroundColor: theme.surfaceStrong, borderColor: theme.border }]}>
            <Text style={[styles.readerTitle, { color: theme.text, fontSize: 24 * appFontScale, lineHeight: 34 * appFontScale }]}>{prayer.title}</Text>
            <Text style={[styles.readerSubtitle, { color: theme.textMuted, fontSize: 14 * appFontScale }]}>{prayer.subtitle}</Text>
            <Text
              style={[
                styles.bodyText,
                { color: theme.text, fontSize: baseFontSize, lineHeight: baseLineHeight, textAlign: textAlignMode },
              ]}
            >
              {prayer.body}
            </Text>
...
const styles = StyleSheet.create({
  ...
  screen: { flex: 1, backgroundColor: '#06164B' },
  contentContainer: { paddingHorizontal: 16, paddingTop: 14, paddingBottom: 120 },
  headerRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 },
  headerLeft: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  headerButton: { width: 38, height: 38, borderRadius: 999, alignItems: 'center', justifyContent: 'center', borderWidth: 1 },
  headerTitle: { flex: 1, color: '#F8FAFC', fontSize: 20, fontWeight: '800', textAlign: 'center' },
  ...
  readerTitle: { color: '#F8FAFC', textAlign: 'center', fontSize: 24, lineHeight: 34, fontWeight: '800', marginBottom: 8 },
  readerSubtitle: { color: '#CBD5E1', textAlign: 'center', fontSize: 14, marginBottom: 18 },
  bodyText: { color: '#F8FAFC', fontWeight: '700', letterSpacing: 0.2 },
  bottomControls: { position: 'absolute', left: 14, right: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  leftControls: {
  ...
  typePreview: { flexDirection: 'row', alignItems: 'flex-end', gap: 8 },
  typePreviewSmall: { color: '#F8FAFC', fontSize: 18, fontWeight: '500' },
  typePreviewValue: { color: '#CBD5E1', fontSize: 12, fontWeight: '700', marginBottom: 3 },
  typePreviewLarge: { color: '#F8FAFC', fontSize: 28, lineHeight: 28, fontWeight: '500' },
  missingWrap: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 24 },
  missingTitle: { color: '#F8FAFC', fontSize: 24, fontWeight: '800', marginBottom: 18 },
  backHomeButton: { paddingHorizontal: 18, paddingVertical: 14, borderRadius: 999, backgroundColor: '#1D4ED8' },
  backHomeText: { color: '#F8FAFC', fontWeight: '700' },
});
